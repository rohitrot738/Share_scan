...
def _pre_breakout_bonus(tf: str, df: pd.DataFrame) -> float:
    """Generic pre-breakout score derived from separate training-case rules."""
    ...
    rules = YATHARTH_RULES
    bonus = 0.0
    if width_pct <= rules['base_width_pct']['strong']:
        bonus += 3.0
    elif width_pct <= rules['base_width_pct']['moderate']:
        bonus += 1.5

    if 0.0 <= distance_pct <= rules['resistance_proximity_pct']['strong']:
        bonus += 5.0
    elif distance_pct <= rules['resistance_proximity_pct']['moderate']:
        bonus += 3.0

    if vol_ratio >= rules['volume_ratio']['strong']:
        bonus += 4.0
    elif vol_ratio >= rules['volume_ratio']['moderate']:
        bonus += 2.0

    limit = rules['extension_limit_pct'].get(tf, rules['extension_limit_pct']['default'])
    if one_bar_move >= limit:
        bonus -= 5.0
    return bonus
...
def analyse_timeframes(data: Dict[str, pd.DataFrame], cfg: ScannerConfig):
    ...
    for tf, df in data.items():
        ...
        score = dict(score_features(features, cfg))
    ...
    return {
        ...
        'training_case': YATHARTH_CASE_ID,
        'timeframes': per_tf,
    }
