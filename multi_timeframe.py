...
def analyse_timeframes(data:Dict[str,pd.DataFrame],cfg:ScannerConfig):
    per_tf={}; weighted=used=0.0; skipped={}
    for tf,df in data.items():
        try:
            ...
            skipped[tf]=f"{type(e).__name__}: {e}"
    if not per_tf:
        raise ValueError('No valid timeframe after indicator calculation: '+str(skipped))
    ...
    return {'final_score':round(final,2),'final_state':state,'high_tf_ok':high_ok,'trigger_ok':trigger_ok,'training_case':YATHARTH_CASE_ID,'timeframes':per_tf,'used_timeframes':list(per_tf.keys()),'skipped_timeframes':skipped}
