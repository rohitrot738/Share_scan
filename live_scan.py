...
DEPTH_LIMIT=max(25,int(os.getenv("SCAN_DEPTH_LIMIT","10")))
...
def daily_prefilter_score(df: pd.DataFrame) -> dict:
    ...
    momentum=max(0.0,min(100.0,50.0+ret5*4.0+ret20*1.5)); liquidity=max(0.0,min(100.0,20.0+math.log10(max(v*c,1.0))*4.0)); volume_score=max(0.0,min(100.0,rvol*25.0)); score=.45*liquidity+.30*volume_score+.25*momentum
...
def _blend_orderbook(out):
    ...
    if not mask.any():
        out['orderbook_adjustment']=0.0; return out
    ...
    out['orderbook_adjustment']=0.0; out.loc[mask,'orderbook_adjustment']=(blended-base).round(2); out.loc[mask,'rank_score']=blended.clip(0,100)
    return out
...
def stage2(shortlisted,top_n,started):
    ...
    out=pd.DataFrame(rows); out["rank_score"]=(out.rank_score-.12*out.false_breakout_risk.fillna(0)).clip(lower=0); out=apply_depth(out,min(DEPTH_LIMIT,len(out))); out=_blend_orderbook(out); q=out.sort_values("rank_score",ascending=False).head(top_n).sort_values(["volume","rank_score"],ascending=[False,False]).reset_index(drop=True); q.insert(0,"volume_rank",np.arange(1,len(q)+1)); return q

def save_results(top,s1,runtime):
    OUTPUT_DIR.mkdir(parents=True,exist_ok=True); top.to_csv(OUTPUT_DIR/"top100_by_volume.csv",index=False); s1.to_csv(OUTPUT_DIR/"stage1_shortlist.csv",index=False); payload={"generated_at":datetime.now().astimezone().isoformat(),"mode":"NSE TRUE ORDERBOOK BRANCH","target_seconds":TARGET_SECONDS,"runtime_seconds":round(runtime,2),"cache_used":True,"shortlist_size":len(s1),"count":len(top),"results":top.replace({np.nan:None}).to_dict("records")}; (OUTPUT_DIR/"top100_by_volume.json").write_text(json.dumps(payload,indent=2,ensure_ascii=False),encoding="utf-8")

def main():
    t0=time.perf_counter(); p=argparse.ArgumentParser(); p.add_argument("--top",type=int,default=100); p.add_argument("--shortlist",type=int,default=500); a=p.parse_args(); s1=load_stage1_cache(max(a.shortlist,a.top)); print(f"Stage-1 shortlist: {len(s1)}"); top=stage2(s1,a.top,t0); runtime=time.perf_counter()-t0; save_results(top,s1,runtime); print(f"NSE orderbook scan complete: {len(top)} in {runtime:.1f}s")
