...
def daily_prefilter_score(df: pd.DataFrame) -> Dict[str,float]:
    ...
    return {"score":round(max(0,min(100,score)),2),"close":round(c,2),"resistance":round(resistance,2),"support":round(support,2),"current_volume":int(vol),"avg_volume20":int(avg20),"rvol":round(rvol,2),"turnover_proxy":round(turnover,2),"distance_to_20d_high_pct":round(dist,2)}


def _scan_daily_batch(batch, by_symbol):
    rows=[]
    try: data=download_batch(batch,period="3mo",interval="1d",retries=0)
...
def _analyse_from_bulk(r,d15,d1h,cfg):
    ...
    mtf=analyse_timeframes(tf,cfg); base=tf.get("15m") if "15m" in tf else tf.get("1h")
    ghost=ghost_trade_snapshot(base) if base is not None and len(base)>=60 else {}; gs=_safe_float(ghost.get("ghost_score",0)); ms=_safe_float(mtf.get("final_score",0)); ps=_safe_float(r.get("score",0))
    plan=ghost.get("trade_plan",{}) if isinstance(ghost,dict) else {}; fb=ghost.get("false_breakout",{}) if isinstance(ghost,dict) else {}
    return {"symbol":r["symbol"],"exchange":r["exchange"],"name":r.get("name",""),"price":r.get("close",0),"volume":int(_safe_float(r.get("current_volume",0))),"avg_volume20":int(_safe_float(r.get("avg_volume20",0))),"rvol_daily":r.get("rvol",0),"rank_score":round(.55*ms+.25*gs+.20*ps,2),"mtf_score":round(ms,2),"mtf_state":mtf.get("final_state",""),"ghost_score":round(gs,2),"ghost_signal":ghost.get("signal",""),"false_breakout_risk":_safe_float(fb.get("risk",0)),"daily_support":r.get("support",0),"daily_resistance":r.get("resistance",0),"entry":plan.get("entry"),"stop":plan.get("stop"),"target1":plan.get("target1"),"target2":plan.get("target2"),"timeframes_used":",".join(sorted(tf)),"analysis_tier":"DEEP"}


def _prefilter_rows(df):
    return [{"symbol":r["symbol"],"exchange":r["exchange"],"name":r.get("name",""),"price":r.get("close",0),"volume":int(_safe_float(r.get("current_volume",0))),"avg_volume20":int(_safe_float(r.get("avg_volume20",0))),"rvol_daily":r.get("rvol",0),"rank_score":round(_safe_float(r.get("score",0)),2),"mtf_score":np.nan,"mtf_state":"PREFILTER","ghost_score":np.nan,"ghost_signal":"","false_breakout_risk":0.0,"daily_support":r.get("support",0),"daily_resistance":r.get("resistance",0),"entry":None,"stop":None,"target1":None,"target2":None,"timeframes_used":"1d","analysis_tier":"PREFILTER"} for r in df.to_dict("records")]
...
def apply_depth(out,limit=10):
    ...
    try:
        rows=out.sort_values("rank_score",ascending=False).head(limit); depth=fetch_depth_scores([(str(r.exchange),str(r.symbol)) for r in rows.itertuples(index=False)])
    except Exception as exc:print(f"[WARN] depth fallback: {exc}"); depth={}
...
def stage2_30s(shortlisted,top_n=100,started=None):
    deep=shortlisted.head(min(DEEP_LIMIT,len(shortlisted))).copy(); symbols=deep["yahoo_symbol"].astype(str).tolist(); elapsed=time.perf_counter()-started if started else 0
    d15={}; d1h={}
    if TARGET_SECONDS-elapsed>8 and symbols:
        try:d15=download_batch(symbols,"10d","15m",retries=0)
        except Exception as exc:print(f"[WARN] 15m bulk skipped: {exc}")
    if TARGET_SECONDS-(time.perf_counter()-started)>8 and symbols:
        try:d1h=download_batch(symbols,"60d","60m",retries=0)
    ...
    deep_syms={r["symbol"] for r in deep_rows}; rows=deep_rows+_prefilter_rows(shortlisted[~shortlisted["symbol"].isin(deep_syms)])
    if not rows:return pd.DataFrame()
    out=pd.DataFrame(rows); out["rank_score"]=(out["rank_score"]-.12*out["false_breakout_risk"].fillna(0)).clip(lower=0)
    depth_limit=min(DEPTH_LIMIT,len(out)) if TARGET_SECONDS-(time.perf_counter()-started)>6 else 0
    out=apply_depth(out,depth_limit); q=out.sort_values("rank_score",ascending=False).head(top_n).copy(); q=q.sort_values(["volume","rank_score"],ascending=[False,False]).reset_index(drop=True); q.insert(0,"volume_rank",np.arange(1,len(q)+1)); return q
