...
def daily_prefilter_score(df: pd.DataFrame) -> Dict[str, float]:
    """Cheap stage-1 score for the entire market using only daily candles.

    Goal: keep liquid, structurally strong names near a breakout/reversal area and
    avoid wasting intraday requests on thousands of weak/illiquid scrips.
    """
...
def stage1(universe: List[Instrument], batch_size: int = 150, shortlist: int = 120) -> pd.DataFrame:
    ...
    out = out.sort_values(["score", "turnover_proxy"], ascending=[False, False])
    ...
    for _, r in shortlisted.iterrows():
        ...
        try:
            ...
            rows.append({
                ...
                "rank_score": round(final_rank, 2),
                "mtf_score": round(mtf_score, 2),
                "mtf_state": mtf.get("final_state", ""),
                "ghost_score": round(ghost_score, 2),
                "ghost_signal": ghost.get("signal", ""),
                "false_breakout_risk": _safe_float(fb.get("risk", 0.0)),
                "daily_support": r.get("support", 0.0),
                "daily_resistance": r.get("resistance", 0.0),
                "distance_to_20d_high_pct": r.get("distance_to_20d_high_pct", 0.0),
                "rvol_daily": r.get("rvol", 0.0),
...
def save_results(top: pd.DataFrame, shortlist_df: pd.DataFrame) -> None:
    ...
    payload = {
        ...
        "count": int(len(top)),
        "results": top.replace({np.nan: None}).to_dict(orient="records") if not top.empty else [],
    }
    (OUTPUT_DIR / "top10.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")


def main():
    p = argparse.ArgumentParser(description="Share_scan live NSE/BSE two-stage scanner")
    ...
    if top.empty:
        ...
        cols = ["symbol", "exchange", "price", "rank_score", "mtf_state", "ghost_signal", "false_breakout_risk", "daily_support", "daily_resistance"]
