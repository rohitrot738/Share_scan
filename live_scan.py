...
def _cache_fresh():return CACHE_FILE.exists() and (time.time()-CACHE_FILE.stat().st_mtime)<CACHE_MAX_AGE_HOURS*3600

def load_stage1_cache(shortlist:int)->pd.DataFrame:
    if not _cache_fresh():raise SystemExit("NSE stage1 cache missing/stale. Run cache builder first; live scan will not cold-build history.")
    df=pd.read_csv(CACHE_FILE); need={"symbol","exchange","yahoo_symbol","score","turnover_proxy","current_volume"}
...
def _longterm_metrics(df):
    if df is None or len(df)<252:return {"week52_position":np.nan,"week156_position":np.nan,"longterm_score":50.0}
    c=pd.to_numeric(df['close'],errors='coerce').dropna(); hi=pd.to_numeric(df['high'],errors='coerce'); lo=pd.to_numeric(df['low'],errors='coerce')
    if c.empty:return {"week52_position":np.nan,"week156_position":np.nan,"longterm_score":50.0}
    def pos(n):
        h=float(hi.tail(min(n,len(hi))).max()); l=float(lo.tail(min(n,len(lo))).min()); x=float(c.iloc[-1]); return 50.0 if h<=l else max(0.0,min(100.0,(x-l)/(h-l)*100.0))
    p52=pos(252); p156=pos(756) if len(c)>=504 else p52
    # Favor stocks in the upper-middle of long structures without rewarding extreme extension.
    def s(p):return 95.0 if 60<=p<=88 else 78.0 if 45<=p<60 else 68.0 if 88<p<=95 else 40.0 if 25<=p<45 else 25.0
    return {"week52_position":round(p52,2),"week156_position":round(p156,2),"longterm_score":round(.45*s(p52)+.55*s(p156),2)}

def apply_longterm(out, shortlisted):
    out['week52_position']=np.nan; out['week156_position']=np.nan; out['longterm_score']=50.0
    syms=shortlisted.head(min(DEEP_LIMIT,len(shortlisted))).yahoo_symbol.astype(str).tolist()
    try:data=download_batch(syms,"5y","1d",retries=0)
    except Exception as e:print(f"[WARN] 52/156-week history fallback: {e}"); return out
    ymap=dict(zip(shortlisted.symbol.astype(str),shortlisted.yahoo_symbol.astype(str)))
    for idx,row in out.iterrows():
        m=_longterm_metrics(data.get(ymap.get(str(row.symbol),''),pd.DataFrame()))
        for k,v in m.items():out.at[idx,k]=v
    # 10% long-term branch contribution, preserving 90% of the tested base rank.
...
def save_results(top,s1,runtime):
    OUTPUT_DIR.mkdir(parents=True,exist_ok=True); top.to_csv(OUTPUT_DIR/"top100_by_volume.csv",index=False); s1.to_csv(OUTPUT_DIR/"stage1_shortlist.csv",index=False); payload={"generated_at":datetime.now().astimezone().isoformat(),"mode":"NSE 52W+156W BRANCH","target_seconds":TARGET_SECONDS,"runtime_seconds":round(runtime,2),"cache_used":True,"shortlist_size":len(s1),"count":len(top),"results":top.replace({np.nan:None}).to_dict("records")}; (OUTPUT_DIR/"top100_by_volume.json").write_text(json.dumps(payload,indent=2,ensure_ascii=False),encoding="utf-8")

def main():
    t0=time.perf_counter(); p=argparse.ArgumentParser(); p.add_argument("--top",type=int,default=100); p.add_argument("--shortlist",type=int,default=500); a=p.parse_args(); s1=load_stage1_cache(max(a.shortlist,a.top)); print(f"Stage-1 shortlist: {len(s1)}"); top=stage2(s1,a.top,t0); runtime=time.perf_counter()-t0; save_results(top,s1,runtime); print(f"NSE 52/156-week scan complete: {len(top)} in {runtime:.1f}s")
