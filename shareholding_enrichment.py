...
def extract(symbol):
    ...
    r=requests.get(url,headers=HEADERS,timeout=12); r.raise_for_status(); tables=pd.read_html(r.text)
    for t in tables:
...
def score(d):
    ...
    x=sum(changes); return max(0,min(100,50+x*18))
...
def main():
    ...
    if JSON.exists():
        payload=json.loads(JSON.read_text(encoding='utf-8')); payload['mode']='NSE FII-DII SHAREHOLDING BRANCH'; payload['results']=df.where(pd.notna(df),None).to_dict('records'); JSON.write_text(json.dumps(payload,indent=2,ensure_ascii=False),encoding='utf-8')
