...
def _session() -> requests.Session:
    ...
    s.mount("https://", adapter); s.mount("http://", adapter); s.headers.update(HEADERS)
    return s
...
def _clean_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not set(required).issubset(x.columns): return pd.DataFrame()
...
def fetch_nse_universe(session: Optional[requests.Session] = None) -> List[Instrument]:
    ...
    try:
        r = s.get(NSE_EQUITY_CSV, timeout=(10, 30)); r.raise_for_status()
        if not r.text.strip(): return []
        df = pd.read_csv(io.StringIO(r.text))
    except Exception as exc:
        print(f"[WARN] NSE universe unavailable: {exc}"); return []
    df.columns = [str(c).strip() for c in df.columns]
    if "SYMBOL" not in df.columns:
        print("[WARN] NSE universe format changed: SYMBOL column missing"); return []
    out = []
...
def _normalize_bse_rows(payload) -> list:
    ...
    for _ in range(3):
        ...
        if isinstance(rows, dict): rows = rows.get("Table") or rows.get("table") or rows.get("data") or rows.get("Data") or []
        else: break
    return rows if isinstance(rows, list) else []


def fetch_bse_universe(session: Optional[requests.Session] = None) -> List[Instrument]:
    s = session or _session(); params = {"Group":"","Scripcode":"","industry":"","segment":"Equity","status":"Active"}
    try:
        r = s.get(BSE_ACTIVE_API, params=params, timeout=(10,35)); r.raise_for_status(); payload = r.json()
    except Exception as exc:
        print(f"[WARN] BSE universe unavailable: {exc}"); return []
    out = []
    for row in _normalize_bse_rows(payload):
        ...
        if code.isdigit(): out.append(Instrument(code, "BSE", f"{code}.BO", name))
    if not out: print("[WARN] BSE universe returned no usable equity rows")
    return out
...
def load_extra_symbols(path: Optional[str] = None) -> List[Instrument]:
    ...
    try: df = pd.read_csv(path)
        ...
        print(f"[WARN] EXTRA_SYMBOLS_FILE unreadable: {exc}"); return []
    df.columns = [str(c).strip().lower() for c in df.columns]; out=[]
    for _, row in df.iterrows():
        exchange = str(row.get("exchange", "NSE")).upper().strip(); symbol = str(row.get("symbol", "")).strip()
        if not symbol or symbol.lower() == "nan": continue
        yahoo = str(row.get("yahoo_symbol", "")).strip()
        if not yahoo or yahoo.lower() == "nan": yahoo = f"{symbol}.NS" if exchange == "NSE" else f"{symbol}.BO"
...
def build_universe(include_nse: bool = True, include_bse: bool = True) -> List[Instrument]:
    s = _session(); items=[]
    if include_nse: items.extend(fetch_nse_universe(s))
    if include_bse: items.extend(fetch_bse_universe(s))
    items.extend(load_extra_symbols()); dedup={}
    for inst in items: dedup[(inst.exchange, inst.yahoo_symbol)] = inst
    return list(dedup.values())


def _yf_download(symbols: List[str], period: str, interval: str, threads: bool, timeout: int = 30):
    return yf.download(tickers=" ".join(symbols), period=period, interval=interval, group_by="ticker",
                       auto_adjust=False, threads=threads, progress=False, timeout=timeout)
...
def download_batch(symbols: Iterable[str], period: str, interval: str, retries: int = 2) -> Dict[str, pd.DataFrame]:
    ...
    for attempt in range(retries+1):
        try:
            raw = _yf_download(symbols, period, interval, threads=True, timeout=25)
            if raw is not None and not raw.empty: break
        except Exception as exc: print(f"[WARN] Yahoo batch attempt {attempt+1} failed ({len(symbols)} symbols): {exc}")
        if attempt < retries: time.sleep(1.5*(attempt+1))
    if raw is not None and not raw.empty:
        if len(symbols)==1: result[symbols[0]] = _clean_ohlcv(raw)
        else:
            for sym in symbols:
...
def fetch_history(symbol: str, period: str, interval: str, retries: int = 2) -> pd.DataFrame:
    ...
    for attempt in range(retries+1):
        try:
            raw=yf.download(symbol, period=period, interval=interval, auto_adjust=False, progress=False, threads=False, timeout=20)
            last=_clean_ohlcv(raw)
            if not last.empty: return last
        except Exception as exc:
            if attempt==retries: print(f"[WARN] Yahoo history failed {symbol} {interval}: {exc}")
        if attempt<retries: time.sleep(min(4.0, 1.0*(2**attempt)))
    return last


def resample_ohlcv(df: pd.DataFrame, rule: str) -> pd.DataFrame:
    if df is None or df.empty or not isinstance(df.index, pd.DatetimeIndex): return pd.DataFrame()
    try:
        return df.resample(rule).agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna(subset=["open","high","low","close"])
    except Exception as exc:
        print(f"[WARN] resample {rule} failed: {exc}"); return pd.DataFrame()


def fetch_superfast_multitimeframe(symbol: str) -> Dict[str, pd.DataFrame]:
    """Latency-first path for live scanning: only the most useful 15m/1h/1d frames."""
    data={}
    for tf,period,interval in [("15m","30d","15m"),("1h","180d","60m"),("1d","6mo","1d")]:
        df=fetch_history(symbol, period, interval, retries=1)
        if len(df)>=60: data[tf]=df
    return data
...
def fetch_multitimeframe(symbol: str) -> Dict[str, pd.DataFrame]:
    ...
    for tf,period,interval in [("15m","60d","15m"),("30m","60d","30m"),("1h","730d","60m"),("1d","2y","1d"),("1w","5y","1wk")]:
        try:
            ...
            if len(df)>=60: data[tf]=df
        except Exception as exc: print(f"[WARN] timeframe {tf} skipped for {symbol}: {exc}")
    if "1h" in data:
        four_h=resample_ohlcv(data["1h"],"4h")
        if len(four_h)>=60: data["4h"]=four_h
    return data
